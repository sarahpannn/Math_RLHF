...
def create_dataset(local_rank, dataset_name, data_split, output_path,
    ...
    print('FIRST TIME TRAIN LEN: ', len(train_dataset))
    train_index = get_raw_dataset_split_index(local_rank, output_path,
                                              raw_dataset.dataset_name_clean,
                                              seed, "train", data_split,
                                              train_phase - 1,
                                              len(train_dataset))
    train_index = [i for i in range(len(train_dataset))]
    train_dataset = Subset(train_dataset, train_index)
    print('TRAIN INDEX: ', len(train_index))
    print('SECOND TIME TRAIN LEN: ', len(train_dataset))
    train_dataset = create_dataset_split(train_dataset, raw_dataset,
                                         train_phase, tokenizer,
                                         end_of_conversation_token,
                                         max_seq_len)
    print('THIRD TIME TRAIN LEN: ', len(train_dataset))

    eval_dataset = raw_dataset.get_eval_data()
    print('FIRST TIME EVAL LEN: ', len(eval_dataset))
    eval_index = get_raw_dataset_split_index(local_rank, output_path,
                                             raw_dataset.dataset_name_clean,
                                             seed, "eval",
                                             data_split, train_phase - 1,
                                             len(eval_dataset))
    eval_index = [i for i in range(len(eval_dataset))]
    eval_dataset = Subset(eval_dataset, eval_index)
    eval_dataset = create_dataset_split(eval_dataset, raw_dataset, train_phase,
                                        tokenizer, end_of_conversation_token,
                                        max_seq_len)
    print("FINAL EVAL LEN: ", len(eval_dataset))
...
class MiniDataset:
    ...
    def seperate(self):
        small_dataset = []
        # print('small batch size: ', self.small_batch_size)
